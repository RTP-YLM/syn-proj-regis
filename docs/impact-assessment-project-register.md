...
| สถานะเอกสาร | Business requirement/workflow (หัวข้อ 1, 2, 1.3–1.5, ภาคผนวก D) **ยังใช้ได้ทั้งหมด ไม่เปลี่ยน** — หัวข้อ 3–9 + ภาคผนวก A–C **เขียนใหม่ให้ตรง stack ใหม่** (21 ก.ค. 2026) — ✅ **ปิด spec ส่วน auth/access-control แล้ว** ตามเอกสาร SSO integration guide (ดู 9c) — 🆕 22 ก.ค. 2026: เพิ่ม scope mobile + LINE (ดู 0d) — เปิดคำถามใหม่ 9b.11–9b.15 — 🆕 22 ก.ค. 2026: เพิ่ม scope AI Chat Assistant (ดู 0e) — เปิดคำถามใหม่ 9b.16–9b.21 — 🆕 23 ก.ค. 2026: เพิ่ม mockup UI (ดู 4.6) — เป็นแค่ภาพประกอบ ไม่ปิดคำถามค้าง — 🆕 **27 ก.ค. 2026 (รอบ 10): ปิดคำถามจากรอบ review 8 ข้อ (ดู 0f) — ตาราง Project Management ยึด Template Excel เป็นต้นแบบ (A.8 เขียนใหม่), Project identity เหลือ 2 field, เพิ่ม revision ระดับ Project; เปิดคำถามใหม่ 1 ข้อ (9b.22)** — 🆕 **30 ก.ค. 2026 (รอบ 11): ตัด AI Chat Assistant ออกจากขอบเขต (0e ยุบเหลือหมายเหตุ, ถอด 4.5/6.5/A.15/endpoint 26-26b/ความเสี่ยง 8.19-8.24 เดิม/คำถาม 9b.16-9b.21/scenario D32-D35 ออกทั้งหมด) + เพิ่ม Telegram เป็นช่องทาง push ที่ 2 คู่กับ LINE — เปิดคำถามใหม่ 9b.23-9b.25** — 🆕 **7 ส.ค. 2026 (รอบ 12 — ดู 0h):** ปิดคำถาม infra/mobile/LINE/PM แทบทั้งหมด (9b.4–9b.15, 9b.22) เหลือเปิดเฉพาะ **9b.23–9b.25 (Telegram bot เลือกตัวไหน/กลไก inbound/channel preference)** — ไม่มีคำถามเหลือที่ blocking การเริ่ม coding อีกแล้วนอกจากรายละเอียด infra ของ Telegram inbound |
...
| Repo | ใช้ repo เดิม 2 ตัว | **Repo ใหม่ — ✅ Mono-repo เดียว (ปิดแล้ว 7 ส.ค. 2026 ดู 0h.3), ชื่อ repo ยังไม่กำหนด (ไม่ blocking)** |
...
| 0d.3 | แจ้งเตือนอนุมัติผ่าน LINE แบบ Flex Message **+ Telegram (เพิ่ม 30 ก.ค. 2026)** | ช่องทางแจ้งเตือน **เพิ่มเติม** จากกระดิ่งในแอป (ไม่ใช่แทนที่) — ผูก LINE Official Account (Messaging API — **OA ใหม่แยกเฉพาะระบบนี้ + LINE Login OAuth สำหรับผูกบัญชี ✅ ปิดแล้ว ดู 0h.10/0h.11**) **และ Telegram Bot API** (กลไก bot/ผูกบัญชียังเปิดอยู่ — 9b.23/9b.24) เข้ากับระบบ event notification ที่มีอยู่แล้ว (`project.notification`, ข้อ 9.24) ผ่าน **channel adapter ตัวเดียว** — รูปแบบข้อความต่างกันคนละชุด (LINE = Flex Message JSON / Telegram = HTML หรือ MarkdownV2 + inline keyboard) จึงต้องแยกชั้น template ต่อช่องทาง ไม่ hardcode (ดู 0g) |
| 0d.4 | ออกแบบสวยงาม เข้าใจงานง่าย | ✅ Design system ปิดแล้ว 7 ส.ค. 2026: **Ant Design** (ดู 0h.4) — เลือกส่วนหนึ่งเพราะรองรับ mobile-first/responsive ในตัว |
...
| 0g.5 | **ผลต่อประมาณการงาน** | −18–27 man-day (chat ทั้ง 4 ก้อน) และ +2–3 man-day (Telegram adapter — ถูกกว่า LINE เพราะ dispatch service/ตาราง/หน้าผูกบัญชีใช้ร่วมกันแล้ว) → **~95–143 → ~79–119 man-day** และเหลือคำถาม blocking แค่ ORM (9b.4) + design system (9b.7) — **ทั้งสองข้อปิดแล้วในรอบ 12 ถัดไป (ดู 0h)** |
...
> **ถ้าจะรื้อ chat กลับมาทำภายหลัง:** ดีไซน์ฉบับเต็ม (tool-use ผูก endpoint เดิม, บังคับ authorization ต่อ tool call, กัน hallucination, FR-01–FR-11) อยู่ใน git history commit `748d2bc` และ openspec design D11 เดิม — ไม่ต้องออกแบบใหม่ ให้ดึงกลับมาแล้วปิดคำถาม LLM provider/DPA ก่อนเริ่ม — **เพิ่มเติมจาก interview รอบ 12 (7 ส.ค. 2026):** `pgvector` extension เตรียมพร้อมใช้งานระดับ infra ของ PostgreSQL ไว้แล้ว (เปิดใช้ได้ทันทีโดยไม่กระทบ schema ปัจจุบัน หากรื้อกลับมาแล้วต้องการ RAG) และมีแนวคิดเบื้องต้นเรื่อง retention ของ conversation log ไว้คุยกันคือกำหนดจำนวนวันผ่านตาราง config แบบ `lov_config` แทนการ hardcode — ทั้งสองเป็นแค่โน้ตอ้างอิง ไม่ใช่การตัดสินใจที่ปิดแล้ว

---

## 0h. รอบ 12 (7 ส.ค. 2026) — ปิดคำถามค้าง Infra/Mobile/LINE/PM ที่เหลือ

ทำ stakeholder interview (grilling session) กับผู้ใช้ ต่อยอดจากรอบ 11 (0g) ปิดคำถามคงค้างเกือบทั้งหมดที่เหลือในหัวข้อ 9b (9b.4–9b.15, 9b.22) — ไม่แตะเรื่อง AI Chat/Telegram ที่ปิดไปแล้วในรอบ 11 ยกเว้นให้รายละเอียดเพิ่มเติมเรื่อง forward-compat ของ AI Chat (ดู 0g ท้ายหัวข้อ)

| # | ประเด็น | ข้อสรุปรอบนี้ | จุดที่แก้ |
|---|---|---|---|
| 0h.1 | ORM (9b.4) | **Prisma** | 5, A.14, 9b |
| 0h.2 | ที่เก็บไฟล์แนบ (9b.5) | **Local disk storage บน server on-prem ที่มีอยู่แล้ว** (ไม่ใช้ object storage) | 6.1, 8.5, B ข้อ 24, 9b |
| 0h.3 | Repo topology (9b.6) | **Mono-repo** | 9b |
| 0h.4 | Design system (9b.7) | **Ant Design** | 4.1, 9b |
| 0h.5 | แหล่งข้อมูลภูมิศาสตร์ไทย (9b.8) | ยืนยัน seed จาก public open dataset ตามสมมติฐานเดิม | 6.1, 8.13, 9b |
| 0h.6 | Hosting (9b.9) | **ใช้ server on-prem ที่มีอยู่แล้วขององค์กร** — รายละเอียด reverse proxy/public callback (จำเป็นสำหรับ SSO callback ที่ปิดแล้วและ LINE Login OAuth ใหม่ รวมถึง webhook ของ Telegram ถ้าเลือกทางนั้น — 9b.24) ไปจัดการตอน design infra แยกต่างหาก ทีมมีประสบการณ์ตั้งค่าลักษณะนี้มาก่อน ไม่ block spec นี้ | 9b |
| 0h.7 | Data migration จาก Syndome CRM เดิม (9b.10) | **ยืนยันมีข้อมูลต้อง migrate จริง** — โครงการที่สถานะยังเปิดอยู่ในระบบเดิมต้อง migrate ให้เสร็จก่อนตัด switch ไปใช้ระบบใหม่ ส่วนประวัติที่ปิดจบแล้วทำเป็น batch job แยกทีหลังได้ ไม่ block go-live | 9b, §10 (เพิ่มงาน migration แยกจากประมาณการ feature) |
| 0h.8 | ทีมคุ้นเคย stack ใหม่แค่ไหน (ความเสี่ยง 8.12) | **ทีมมีความรู้ stack ใหม่แล้ว** — ตัด ramp-up time caveat ออกจากหัวข้อ 10 | 8.12, §10 |
| 0h.9 | Mobile display pattern ตารางหนาแน่น (9b.11) | **Card-based layout ต่อ record + accordion เปิด/ปิดดู spec line** | 4.4, 8.15, 9b |
| 0h.10 | LINE Official Account (9b.12) | **สร้าง OA ใหม่แยกเฉพาะระบบนี้** | 6.4, 8.16, 9b |
| 0h.11 | วิธีผูกบัญชี LINE (9b.13) | **LINE Login OAuth** — Telegram ยังแยกคำถามอยู่ที่ 9b.24 | 5, 6.4, B ข้อ 25, 9b |
| 0h.12 | LINE Flex Message โต้ตอบได้ไหม (9b.14) | **Push + deep-link เท่านั้น** ไม่มีปุ่มโต้ตอบ ไม่ต้อง postback webhook — คนละประเด็นกับ inbound `/start` ของ Telegram (9b.24) ซึ่งเป็นเรื่อง account-linking ไม่ใช่การโต้ตอบในข้อความแจ้งเตือน | 5, 8.16, 9b |
| 0h.13 | Event ไหนยิง push (9b.15) | **เฉพาะ event ที่ต้อง action จากผู้รับ** (ไม่ push ทุก state change) — ใช้กับทั้ง LINE และ Telegram เหมือนกัน | 5, 6.4, 8.16, 9b |
| 0h.14 | Log สถานะการส่ง push (จาก 6.4 — เดิมเป็นคำถามเปิดของทั้งสองฝั่ง) | **ต้องมี** — ออกแบบเป็นตารางใหม่ **`project.interface_api_logs`** แบบ generic ใช้ log ได้หลาย external interface (เริ่มที่ LINE/Telegram push ตอนนี้ เผื่อ ERP/LLM ในอนาคต) ไม่ทำเป็น log แยกตามช่องทาง | 6 (ใหม่), B |
| 0h.15 | ERP item master linkage (9b.22) | **กรอกมือไปก่อนอย่างเป็นทางการในเฟสนี้** — ปิดเป็น "ยังไม่ต่อ ERP รอบนี้" ไม่ใช่คำถามค้างไม่มีกำหนดอีกต่อไป (deferred เป็นเฟสหน้า) | A.8.2, 9b, §10 (ปลดล็อกการปิดตัวเลข man-day ตาราง PM) |
| 0h.16 | ช่องต้นทุนพิมพ์นิพจน์ได้ (A.8.4 ข้อ 8) | **ไม่รองรับพิมพ์สูตร** — แยกเป็นช่อง `qty × unit price` ให้กรอกแทน | A.8.4 |
| 0h.17 | State machine ขัดกับข้อสรุป 9.17 (หัวข้อ 1.4 แถว `waiting→rejected`) | แก้เป็น **"หัวหน้าไม่อนุมัติ"** เท่านั้น ตัด "Manager" ออกจากแถวนี้ ให้ตรงกับ 9.17/Access Matrix | 1.4 |
| 0h.18 | ประมาณการงาน (หัวข้อ 10) | นำเสนอเป็น **ช่วง (range) พร้อม caveat ต่อบรรทัดที่ยังไม่นิ่ง** แทนตัวเลขเดียว | 10 |

ผลคือคำถาม blocking ที่เหลือจากรอบ 11 (ORM, design system) ปิดครบแล้ว — คำถามค้างที่เหลือทั้งหมดในเอกสารตอนนี้คือ **9b.23–9b.25 (Telegram bot/inbound/channel preference)** เท่านั้น ไม่มีคำถามเปิดเรื่อง AI Chat หลงเหลืออยู่แล้วเพราะฟีเจอร์นั้นถูกตัดออกทั้งก้อนตั้งแต่รอบ 11
...
| `waiting` | หัวหน้าไม่อนุมัติ | `rejected` | บังคับกรอกเหตุผล เก็บลงประวัติ — **แก้ 7 ส.ค. 2026 (0h.17): ตัด "Manager" ออกจากแถวนี้** (ของเดิมเขียนตกค้างจากรอบ flowchart ก่อน 9.17 จะสรุป) ให้ตรงกับข้อสรุปปิดแล้ว 9.17 และ Access Matrix (ภาคผนวก B.2) — ชั้นอนุมัติ Register ตั้งต้นจบที่หัวหน้าคนเดียวเท่านั้น |
...
[React SPA]  ──HTTPS/JSON──▶  [Fastify API]  ──SQL (driver/ORM)──▶  [PostgreSQL]
 ...
 เรียก API ผ่าน fetch/axios    ต่อ endpoint                          สำหรับ auth.user)
     ...
     │                                   ผ่าน channel adapter ตัวเดียว (0d.3 / 0g) — ทั้งคู่บันทึกผลลง interface_api_logs (0h.14)
...
- ~~AI Chat Assistant / LLM API~~ **ตัดออกจาก architecture นี้ทั้งหมด 30 ก.ค. 2026 (ดู 0e/0g)** — ไม่มี external LLM integration ในระบบนี้อีกต่อไป
- **สิ่งที่ต้องตัดสินใจก่อนเริ่ม coding จริง ปิดครบแล้ว (7 ส.ค. 2026 — ดู 0h):** ORM (Prisma), ที่เก็บไฟล์แนบ (local disk บน server on-prem), responsive breakpoint/pattern (card/accordion), LINE OA/การผูกบัญชี (OA ใหม่ + LINE Login OAuth) — **เหลือเฉพาะรายละเอียด infra ของ Telegram (bot เลือกตัวไหน 9b.23, กลไก inbound 9b.24, channel preference 9b.25) ที่ยังไม่ปิด**
...
| Styling/Design system | Prototype เดิมใช้ font Sarabun + CSS มือเขียนแบบ standalone (ไม่ใช่ theme "Sneat" ของระบบเดิมอีกต่อไป เพราะไม่ผูกกับระบบเดิมแล้ว) — ✅ **ปิดแล้ว 7 ส.ค. 2026 (ดู 0h.4): Ant Design** — แปล UX จาก Prototype เข้าไปโดยใช้ component/breakpoint ของ Ant Design เป็นหลัก |
...
- **แนวทาง**: Responsive web ด้วย React ชุดเดียว (ไม่ใช่แอปแยก, ไม่ใช่ codebase คนละชุด) — ใช้ breakpoint ของ Ant Design (✅ เลือกแล้ว 0h.4) กำหนด layout อย่างน้อย 2 ระดับ: desktop (≥ ~1024px) และ mobile (< ~768px) — ทุกหน้า render ได้ทั้ง 2 ระดับจาก component/route เดียวกัน ไม่ทำเป็นชุดหน้าแยกต่างหาก
- **จุดที่ต้องออกแบบใหม่จริงจัง** (ไม่ใช่แค่ CSS responsive ธรรมดา) เพราะ Prototype วางมาสำหรับตาราง desktop กว้างล้วนๆ:
  ...
  - ตาราง Project Management (2 ระดับ + แถวสรุป, 18 คอลัมน์ตาม template — A.8) → **จุดยากที่สุดของงานนี้** บนจอเล็กตารางกว้างขนาดนี้อ่านไม่ได้เลย — ✅ **ปิดแล้ว 7 ส.ค. 2026 (ดู 0h.9): card-based layout ต่อ task + accordion เปิด/ปิดดูตามลำดับชั้น** — ยังต้องทำ mockup ละเอียด + ทดสอบกับผู้ใช้จริงก่อน implement เต็มรูปแบบ (ดูความเสี่ยง 8.15)
...
- **ปิดแล้ว (ดู 0h.9)**: pattern การแสดงผลของตาราง PM/compare บนมือถือ = card/accordion — breakpoint ที่แน่นอนใช้ตามค่า default ของ Ant Design ยังต้องทำ mockup ละเอียด/ทดสอบกับผู้ใช้จริงก่อน build เต็มรูปแบบ
...
**ขอบเขตที่ mockup นี้ยังไม่ครอบคลุม** (สำคัญ — อย่าตีความว่าปิดคำถามไปแล้ว): ไม่ได้แตะ pattern การ์ด/accordion ของตาราง PM (ตาม template) หรือตารางเปรียบเทียบ Entry บนมือถือ ซึ่งเป็นจุดยากที่สุดที่ยังไม่มี mockup ระดับ implement (ทิศทาง design ปิดแล้ว 7 ส.ค. 2026 — ดู 0h.9 — แต่ยังไม่ได้ลงมือทำ mockup ละเอียด); ไม่มี mockup ของ LINE interactive/postback เพราะปิดแล้วว่า**ไม่ทำ** (0h.12); ไม่ได้แตะรายละเอียด Telegram (9b.23–9b.25 ยังเปิดอยู่)
...
| Data access / ORM | ✅ **ปิดแล้ว 7 ส.ค. 2026 (ดู 0h.1): Prisma** — migration tooling ครบ (Prisma Migrate แทนโฟลเดอร์ `Deploy_SQL/<วันที่>/` เดิม), type-safe, เขียนเร็ว |
...
| **(ใหม่ 22 ก.ค. 2026 — ปรับ 30 ก.ค./7 ส.ค. 2026 ดู 0g/0h) Push notification dispatch (LINE + Telegram)** | บริการใหม่ — ฟังเมื่อมีแถวใหม่ใน `project.notification` ที่เป็นเหตุการณ์ที่เลือกให้ push (✅ ปิดแล้ว — เฉพาะ event ที่ต้อง action ดู 0h.13) แล้วยิงออกตาม**ช่องทางที่ผู้รับผูกไว้**: LINE Messaging API `POST /v2/bot/message/push` (Flex Message) และ/หรือ Telegram Bot API `POST /bot{token}/sendMessage` (HTML/MarkdownV2 + inline keyboard URL button) — ต้องมี mapping ผู้ใช้↔LINE `userId` (LINE Login OAuth ✅ ปิดแล้ว 0h.11) / Telegram `chat_id` (กลไกยังเปิดอยู่ 9b.24) ก่อน — **จัดเป็น channel adapter: business logic รู้แค่ "ส่ง event นี้ให้ user นี้" ส่วนการ render ข้อความและการเรียก API เป็นหน้าที่ของ adapter ต่อช่องทาง** — เป็น **push ทางเดียว** (✅ ปิดแล้ว: ไม่มี logic รับ postback action จาก LINE — ดู 0h.12) — ทำเป็น service แยกเรียกจาก event-notification service เดิม (asynchronous, ไม่ผูกกับ HTTP request ที่ trigger event) กัน response ของ API ช้าเพราะรอ external API ตอบ + **บันทึกผลการส่งทุกครั้งลง `project.interface_api_logs`** (สำเร็จ/ล้มเหลว/quota เกิน — ดู 0h.14/6) แทนการ handle แบบ silent เฉยๆ — push ล้มเหลวช่องทางใดไม่ทำให้ event หลักในระบบล้มเหลวตาม (กระดิ่งในแอปต้องยังขึ้นแม้ push ไม่สำเร็จ) |
| **(ใหม่ — ปรับ 30 ก.ค./7 ส.ค. 2026) Account linking (LINE / Telegram)** | endpoint ใหม่ให้ user ผูกบัญชีของตัวเองกับ `auth.user` — LINE (✅ **กลไกปิดแล้ว: LINE Login OAuth** ดู 0h.11) และ Telegram (**ต้องรับ `/start` ที่มี token จากฝั่ง Telegram = ต้องมี webhook หรือ long-polling ต่างจาก LINE ที่ push ทางเดียวพอ** — ยังเปิดอยู่ ดู 9b.24) — ถ้ายังไม่ผูกช่องทางใดเลย จะไม่ได้รับ push แต่กระดิ่งในแอปยังทำงานปกติ (ไม่ผูกกัน) |
...
- Migration/versioning: ✅ **ปิดแล้ว (ดู 0h.1) — Prisma Migrate** แทนโฟลเดอร์ `Deploy_SQL/<วันที่>/` เดิม

### 6.4 Push channel binding — LINE / Telegram (ใหม่ 22 ก.ค. 2026 — เพิ่ม Telegram 30 ก.ค. 2026 ดู 0d.3/0g)

✅ **LINE ปิดแล้ว 7 ส.ค. 2026: ผูกบัญชีผ่าน LINE Login OAuth** (ดู 0h.11) — Telegram ยังเปิดอยู่ (9b.24)
...
- ✅ **ปิดแล้ว 7 ส.ค. 2026 (ดู 0h.14):** ต้องมีตาราง log สถานะการส่งแยก — ออกแบบเป็นตารางใหม่แบบ generic **`project.interface_api_logs`** (ดู 6.1) ใช้ร่วมกันได้ทั้ง LINE/Telegram/interface อื่นในอนาคต (ERP, ฯลฯ) ไม่ทำเป็น log แยกตามช่องทาง
- **23 ก.ค. 2026:** มี mockup หน้าตา Flex Message ที่จะ push จริงแล้ว (self-authored ดู 4.6) — ช่วยยืนยันภาพ "เนื้อหาน้อยที่สุด" ตามความเสี่ยง 8.18 ให้เป็นรูปธรรม ใช้เป็นต้นแบบเนื้อหาของ Telegram ได้ด้วย (เปลี่ยนแค่ชั้น render) แต่ยังไม่ปิดคำถาม 9b.23–9b.25 ข้อใด

### 6.5 ~~AI Chat Assistant — conversation storage~~ → ✂️ ตัดออก (30 ก.ค. 2026 — ดู 0e/0g)

ไม่มีตาราง `chat_session`/`chat_message` ในระบบนี้ — และไม่มีข้อมูลอ่อนไหวชุดใหม่ที่ต้องกำหนดนโยบาย retention เพิ่มจากที่มีอยู่แล้ว — **เพิ่มเติมจาก interview รอบ 12 (7 ส.ค. 2026) เผื่อรื้อกลับมาทำภายหลัง:** `pgvector` extension มีพร้อมใช้งานระดับ infra ของ PostgreSQL แล้ว (เปิดใช้ได้ทันทีไม่กระทบ schema ปัจจุบัน) และมีแนวคิดเบื้องต้นว่า retention ของ conversation log ควรกำหนดผ่านตาราง config แบบ `lov_config` แทนการ hardcode จำนวนวัน — ทั้งสองเป็นโน้ตอ้างอิงสำหรับ Phase 2 เท่านั้น ไม่ใช่การตัดสินใจที่ปิดแล้ว
...
| 8.12 | ✅ **ปิดแล้ว 7 ส.ค. 2026 (ดู 0h.8)**: ทีมยืนยันมีความรู้ Node/Fastify/React/Postgres แล้ว | ต่ำ | ไม่ต้องกันเวลา ramp-up เพิ่มในประมาณการงาน (หัวข้อ 10) |
| 8.13 | **ต้องหาแหล่งข้อมูลภูมิศาสตร์ไทย (จังหวัด/อำเภอ/ตำบล/ไปรษณีย์) ใหม่** เพราะเดิม reuse lookup API ของ `syndome-crm-api` ได้ฟรี ตอนนี้ standalone แล้วไม่มีให้ reuse | ต่ำ–กลาง | ✅ ปิดแล้ว 7 ส.ค. 2026 (ดู 0h.5): ยืนยัน seed จาก dataset เปิดสาธารณะเข้า PostgreSQL ตอนตั้งระบบครั้งแรก |
...
| 8.16 | **(ใหม่ — ปรับ 30 ก.ค. 2026)** LINE Messaging API push มีโควตาฟรีต่อเดือนตามแพ็กเกจ OA เกินแล้วมีค่าใช้จ่ายเพิ่มหรือถูกจำกัด — ถ้า push ทุก event อาจชนโควตาเร็วกว่าคาด (**Telegram Bot API ไม่มีโควตารายเดือนแบบนี้ มีแต่ rate limit ต่อวินาที จึงเบากว่าในแง่นี้**) | กลาง | ✅ ปิดแล้ว 7 ส.ค. 2026: OA ใหม่แยกเฉพาะระบบนี้ (0h.10) + push เฉพาะ event ที่ต้อง action เท่านั้น (0h.13) ไม่ mirror ทุก event — ลดความเสี่ยงชนโควตาไปมากแล้ว, `project.interface_api_logs` (0h.14) ช่วยเห็นปริมาณ/ความล้มเหลวจริงของทั้ง 2 ช่องทางเพื่อเฝ้าระวังต่อไป — ยังต้องยืนยันแพ็กเกจ/โควตาจริงของ OA ใหม่กับผู้ใช้ |
| 8.17 | **(ใหม่)** ต้องมีขั้นตอนผูกบัญชี LINE/Telegram ↔ ผู้ใช้ในระบบ ไม่ใช่อัตโนมัติ 100% — ถ้า user ไม่ผูกบัญชีช่องทางใดเลย จะไม่ได้รับแจ้งเตือนแบบเงียบๆ (กระดิ่งในแอปยังทำงาน แต่ถ้า user ไม่ค่อยเข้าเว็บอาจพลาดงานอนุมัติ) | กลาง | UI ต้องเตือนชัดเจนตอนยังไม่ผูกบัญชี (เช่น banner "ยังไม่ได้รับแจ้งเตือนผ่าน LINE/Telegram") ไม่ใช่แค่เงียบไป |
...
## 9b. คำถามคงค้างใหม่จาก Tech Stack Pivot (เปิด 21 ก.ค. 2026 — 9b.1–9b.3 ปิดแล้ว 21 ก.ค. 2026 ดู 9c; 9b.16–9b.21 ตัดออกจากทะเบียนนี้ 30 ก.ค. 2026 เพราะ AI Chat Assistant ถูกตัดออกจากขอบเขตทั้งก้อน ดู 0g; 9b.4–9b.15, 9b.22 ปิดแล้ว 7 ส.ค. 2026 ดู 0h; เหลือเปิดเฉพาะ 9b.23–9b.25 เรื่อง Telegram)
...
| 9b.4 | ORM/query layer สำหรับ Postgres (Prisma / Drizzle / Knex+pg / อื่นๆ) | ✅ **ปิดแล้ว 7 ส.ค. 2026 — ดู 0h.1**: **Prisma** | ~~กระทบรูปแบบ migration script...~~ → ปิดแล้ว (ภาคผนวก A.14, หัวข้อ 5 ปรับตาม Prisma) |
| 9b.5 | ที่เก็บไฟล์แนบ (local disk ของ API server vs object storage เช่น S3-compatible) | ✅ **ปิดแล้ว 7 ส.ค. 2026 — ดู 0h.2**: **Local disk storage บน server on-prem ที่มีอยู่แล้ว** (ไม่ใช้ object storage — ทีมยืนยันแล้ว) | ~~กระทบ schema...~~ → ปิดแล้ว (`entry_file.file_path` เป็น path string ของ local disk, endpoint 24 ปรับตาม — ยังต้องวางแผน backup/redundancy ของโฟลเดอร์นี้เองที่ระดับ infra) |
| 9b.6 | Repo topology (mono-repo เดียว vs แยก UI/API เป็น 2 repo) + ชื่อ repo/โปรเจกต์ | ✅ **ปิดแล้ว 7 ส.ค. 2026 — ดู 0h.3**: **Mono-repo** — ชื่อ repo ยังไม่กำหนด (ไม่ blocking) | ~~ไม่กระทบ business spec...~~ → ปิดแล้ว (โครงสร้าง CI/CD วางแผนบน mono-repo เดียว) |
| 9b.7 | Design system/component library ฝั่ง React (MUI/Ant Design/Chakra/Tailwind ฯลฯ) | ✅ **ปิดแล้ว 7 ส.ค. 2026 — ดู 0h.4**: **Ant Design** | ~~กระทบเวลาออกแบบ UI...~~ → ปิดแล้ว (หัวข้อ 4.1 ปรับตาม Ant Design; ตัวเลขหัวข้อ 10 ยังคงกันเวลาออกแบบ UI ไว้เพราะยังต้องแปล UX จาก Prototype) |
| 9b.8 | แหล่งข้อมูลภูมิศาสตร์ไทย (จังหวัด/อำเภอ/ตำบล/ไปรษณีย์) ใหม่ | ✅ **ปิดแล้ว 7 ส.ค. 2026 — ดู 0h.5**: ยืนยัน seed จาก dataset เปิดสาธารณะ ตามสมมติฐานเดิม (ความเสี่ยง 8.13) | ~~กระทบ...~~ → ปิดแล้ว |
| 9b.9 | Hosting/deployment target + CI/CD | ✅ **ปิดแล้ว 7 ส.ค. 2026 — ดู 0h.6**: **Server on-prem ที่องค์กรมีอยู่แล้ว** — รายละเอียด CI/CD pipeline + reverse proxy/public callback exposure (จำเป็นสำหรับ SSO callback ที่ปิดแล้วตาม 9c, LINE Login OAuth ตาม 9b.13 และอาจรวมถึง Telegram webhook ตาม 9b.24) ไปจัดการตอน design infra แยกต่างหาก ทีมมีประสบการณ์มาก่อน | ~~ไม่กระทบ business spec...~~ → ปิดแล้ว — คงมีงาน infra design แยกที่ยังไม่ประมาณการไว้ในหัวข้อ 10 |
| 9b.10 | มีข้อมูลใดจากระบบ Syndome CRM เดิมที่ต้อง migrate/import เข้าระบบใหม่ครั้งเดียวหรือไม่ (เช่น รายชื่อ user/Dealer ที่มีอยู่แล้ว) หรือเริ่มจาก DB ว่างเปล่าทั้งหมด | ✅ **ปิดแล้ว 7 ส.ค. 2026 — ดู 0h.7**: **มีข้อมูลต้อง migrate จริง** — โครงการที่สถานะยังเปิดอยู่ในระบบเดิมต้อง migrate ให้เสร็จก่อนตัด switch ไปใช้ระบบใหม่ ประวัติที่ปิดจบแล้วทำเป็น batch job แยกทีหลังได้ | **งานใหม่ที่ยังไม่ประมาณการในหัวข้อ 10** — ต้องเพิ่มบรรทัด migration แยก (ขอบเขต/จำนวนข้อมูลจริงยังไม่ทราบ) |
| 9b.11 | **(ใหม่ 22 ก.ค. 2026 — ดู 0d.1/0d.2)** Responsive breakpoint + pattern การแสดงผลตารางข้อมูลหนาแน่น (ตาราง PM ตาม template — 18 คอลัมน์ 2 ระดับ, ตารางเปรียบเทียบ Entry) บนจอมือถือ | ✅ **ปิดแล้ว 7 ส.ค. 2026 — ดู 0h.9**: **Card-based layout ต่อ record + accordion เปิด/ปิดดู spec line** — ยังต้องทำ mockup/ทดสอบกับผู้ใช้จริงก่อน implement เต็มรูปแบบ | ~~กระทบเวลาออกแบบ UI...~~ → ทิศทาง design ปิดแล้ว แต่ยังต้องทำ mockup ละเอียดก่อนเริ่ม build จริง (ดูความเสี่ยง 8.15) |
| 9b.12 | **(ใหม่ — ดู 0d.3)** ใช้ LINE Official Account ตัวไหน — OA ที่องค์กรมีอยู่แล้ว (เช่นของบริษัทแม่/CRM เดิม) หรือสร้างใหม่เฉพาะระบบนี้ | ✅ **ปิดแล้ว 7 ส.ค. 2026 — ดู 0h.10**: **สร้าง OA ใหม่แยกเฉพาะระบบนี้** | ~~กระทบ Channel ID/Secret...~~ → ปิดแล้ว (ขอ Channel ID/Secret ของ OA ใหม่ได้เลย ไม่ต้องกังวลโควตาร่วมกับระบบอื่น) |
| 9b.13 | **(ใหม่)** วิธีผูกบัญชี LINE ↔ ผู้ใช้ในระบบ — LINE Login OAuth เต็มรูปแบบ vs ให้ user ยืนยันรหัสผ่านแชทกับ OA vs admin กรอก LINE userId เอง | ✅ **ปิดแล้ว 7 ส.ค. 2026 — ดู 0h.11**: **LINE Login OAuth** | ~~กระทบ endpoint account-linking...~~ → ปิดแล้ว (ต้องขอ LINE Login channel เพิ่มจาก LINE Developers — งาน infra ใหม่ที่ต้องทำคู่กับ OA ใหม่ตาม 9b.12) |
| 9b.14 | **(ใหม่)** แจ้งเตือน LINE เป็น one-way push อย่างเดียว หรือต้องกดอนุมัติ/ไม่อนุมัติได้จากปุ่มใน Flex Message โดยตรง (ต้องมี webhook รับ postback) | ✅ **ปิดแล้ว 7 ส.ค. 2026 — ดู 0h.12**: ยืนยัน **push อย่างเดียว + ปุ่ม deep-link เปิดเว็บ** ตามสมมติฐานเดิม — ไม่ทำ webhook/postback รอบนี้ (คนละประเด็นกับ inbound `/start` ของ Telegram ใน 9b.24) | ~~ถ้าต้องการอนุมัติจากใน LINE จริง...~~ → ปิดแล้ว ไม่ต้องเพิ่ม webhook endpoint ฝั่ง LINE |
| 9b.15 | **(ใหม่)** เหตุการณ์ไหนบ้างที่ต้อง push — เฉพาะ "รออนุมัติ" หรือรวม approved/rejected/won/lost/ล่ม ด้วย | ✅ **ปิดแล้ว 7 ส.ค. 2026 — ดู 0h.13**: ยืนยัน **เฉพาะเหตุการณ์ที่ต้องการ action จากผู้รับ** ตามสมมติฐานเดิม ใช้กับทั้ง LINE และ Telegram | ~~กระทบปริมาณ push...~~ → ปิดแล้ว — **เพิ่มการตัดสินใจใหม่ (0h.14): ต้องมีตาราง `interface_api_logs` เก็บสถานะการส่งจริงของทั้ง 2 ช่องทาง** (ดู 6) |
| ~~9b.16–9b.21~~ | ~~คำถามค้างของ AI Chat Assistant (LLM provider/DPA, retention, streaming, rate limit, real-time stats, tool-use vs RAG)~~ | **Moot — ตัดออกจากทะเบียนนี้ 30 ก.ค. 2026** เพราะ AI Chat Assistant ถูกตัดออกจากขอบเขตโครงการทั้งก้อน (ดู 0e/0g) ไม่ใช่แค่ deferred | ดูรายละเอียดเดิมได้จาก git history (`748d2bc`) ถ้ารื้อฟีเจอร์นี้กลับมาทำใน Phase ถัดไป |
| 9b.22 | **(ใหม่ 27 ก.ค. 2026 — ดู 0f.3)** ช่อง `รายการ`/`Model` ในตาราง Project Management **ดึงจาก item master ของ ERP** — ยังไม่สรุปว่าเชื่อมอย่างไร: ERP ตัวไหน / มี REST API ให้เรียกไหม (ค้นหา + ดึงราคาทุนล่าสุด) / หรือ sync เป็นรอบเข้ามาเก็บใน `project` DB / หรือรอบแรกให้พิมพ์เองไปก่อน | ✅ **ปิดแล้ว 7 ส.ค. 2026 — ดู 0h.15**: **กรอกมือไปก่อนอย่างเป็นทางการในเฟสนี้ทั้งหมด** — ไม่ต่อ ERP รอบนี้ (เดิมค้างไว้แบบไม่มีกำหนด ตอนนี้ปิดเป็นพฤติกรรมที่ shipped จริง) เตรียมคอลัมน์ `erp_item_code` ไว้เฉยๆ เหมือนเดิม | ~~กระทบ: (1) ตัวเลข man-day...~~ → **ปลดล็อกแล้ว**: (1) ตัวเลข man-day ตาราง PM ในหัวข้อ 10 ทบทวนได้เลย ไม่ต้องรอ ERP (2)-(3) ยังไม่เกิดขึ้นในเฟสนี้ เพราะไม่มีการต่อ ERP จริง |
...
| 9b.24 | **(ใหม่ 30 ก.ค. 2026)** วิธีผูกบัญชี Telegram + จะรับ `/start` ด้วย **webhook** (ต้องเปิด inbound HTTPS ให้ Telegram เรียก) หรือ **long-polling** (worker ค้าง ไม่ต้องเปิด port) | สมมติ deep-link `https://t.me/<bot>?start=<token>` โดยระบบออก token ผูกกับ user + หมดอายุ แล้วรับผ่าน webhook | กระทบ infra/hosting (✅ on-prem server ปิดแล้ว 0h.6 — รายละเอียด reverse proxy/inbound ยังต้องคุยกับทีม infra) และความเสี่ยง 8.19 — **ต่างจาก LINE ที่ push ทางเดียวไม่ต้องมี inbound เลย** |
...
| **ออกแบบ design system/component library ตั้งแต่ต้น** (Ant Design — ✅ เลือกแล้ว 0h.4 — ไม่มี theme สำเร็จรูปให้แปลงเหมือนรอบเดิม ยังต้องแปล UX จาก Prototype เข้าไปทั้งหมด — ดู 4.1) | 2–4 |
...
| **(ดู 0d/4.4) Responsive ทุกหน้า + ออกแบบใหม่เฉพาะจุด** (card/accordion pattern สำหรับตาราง PM/compare บนมือถือ — ✅ ทิศทาง design ปิดแล้ว 0h.9 — ยังต้องทำ mockup ละเอียด + build จริง) | 5–8 |
| **(ดู 0d/5/6.4) LINE Messaging integration** (OA ใหม่ + LINE Login OAuth account-linking + Flex Message template ต่อ event ที่ต้อง action + dispatch service แบบ async ผ่าน channel adapter + `interface_api_logs` — ✅ ทุกกลไกฝั่ง LINE ปิดแล้ว 0h.10–0h.14) | 5–7 |
| **(ใหม่ 30 ก.ค. 2026 — ดู 0g) Telegram Bot integration** (adapter ตัวที่ 2 + template HTML/inline keyboard + account-linking ที่ต้องรับ `/start` ผ่าน webhook หรือ long-polling — ถูกกว่า LINE เพราะ dispatch service/ตาราง/UI ผูกบัญชี/`interface_api_logs` ใช้ร่วมกันแล้ว — กลไก inbound ยังไม่เลือก 9b.24) | 2–3 |
| **(ใหม่ 7 ส.ค. 2026 — ดู 0h.7) Data migration จาก Syndome CRM เดิม** (โครงการที่ยังเปิดอยู่ในระบบเดิม — ต้อง migrate ก่อนตัด switch; ประวัติที่ปิดจบแล้วทำ batch แยกได้) — ⚠️ **ตัวเลขหยาบมาก รอ scope/จำนวนข้อมูลจริง** | 3–6 (หยาบมาก) |
| ทดสอบรวม + แก้บั๊ก + UAT (เพิ่ม flow Manager + แพ้/ล่ม + auth ผ่าน SSO — D29 + responsive/LINE/Telegram — D30–D31; **ไม่รวม AI chat scenario D32–D35 อีกต่อไป — ตัดออกจากขอบเขตแล้ว**) | 8–11 |
| **รวม (รวม Auth/SSO + Mobile + LINE + Telegram + Data migration แล้ว — ✂️ ตัด AI Chat Assistant ออกทั้งก้อน — ไม่ต้องกัน ramp-up stack ใหม่แล้วเพราะทีมคุ้นเคย stack นี้อยู่แล้ว)** | **~83–125 man-day** |

> ตัวเลขเป็น effort รวม UI+API ถ้าทำขนาน 2 คน (UI/API) ระยะเวลาปฏิทินประมาณ **~12–17 สัปดาห์รวม UAT** — ประวัติการปรับตัวเลขรอบก่อนหน้า: รอบ 2 เพราะ updated-flow ดึงขั้น Manager/พี่บีเข้ามาทั้งหมด (~43–65 → ~54–78 man-day); รอบ 3/4 (review) เพิ่ม Project-level lifecycle, revision อนุมัติซ้ำ, event notification (~54–78 → ~59–86 man-day); รอบ 5 (re-platform) เพิ่มงาน design system ใหม่ แต่ตัด/ปรับงานที่เคย workaround เฉพาะ .NET ออก — สุทธิใกล้เคียงเดิม (~59–86 → ~61–90 man-day) บวก Auth/SSO ที่ตอนนั้นยัง estimate ไม่ได้; รอบ 6 (ปิด SSO): เพิ่ม Auth/SSO integration 6–9 man-day (~61–90 → ~67–99 man-day); รอบ 7 (Mobile + LINE): เพิ่ม Responsive 5–8 + LINE integration 4–6 man-day (~67–99 → ~76–113 man-day); รอบ 8 (AI Chat Assistant): เคยเพิ่ม Chat orchestration/storage/UI/security รวม 18–27 man-day เข้ารวมยอด (~76–113 → ~95–143 man-day); **รอบ 11 (30 ก.ค. 2026 — ดู 0g): ตัด AI Chat Assistant ออกทั้งก้อน (−18 ถึง −27) + เพิ่ม Telegram เป็นช่องทาง push ที่ 2 (+2 ถึง +3) → ~95–143 → ~79–119 man-day**; **รอบ 12 (7 ส.ค. 2026 — ดู 0h): ปรับ PM table ขึ้นเล็กน้อยจาก roll-up/GP ที่ซับซ้อนกว่าคาด (+1 ถึง +2), ปรับ LINE integration ขึ้นเล็กน้อยจาก `interface_api_logs` (+1), เพิ่ม Data migration ใหม่ที่ไม่เคยประมาณการมาก่อน (+3 ถึง +6) → รวม ~83–125 man-day** — ตัวเลขนี้นิ่งกว่ารอบก่อนมาก เพราะคำถามค้างเกือบทั้งหมดปิดแล้ว เหลือความไม่แน่นอนหลักที่ scope ของ data migration (9b.10) และรายละเอียด infra ของ Telegram (9b.23–9b.25) เท่านั้น

### ลำดับงานแนะนำ

0. **ปิดคำถาม 9b ที่ยัง blocking** — ✅ **ปิดครบแล้วทั้งหมด (7 ส.ค. 2026 — ดู 0h)**: SSO (9b.1–9b.3 — ดู 9c), ORM/design system/repo/geo-data/hosting/data-migration/mobile-pattern/LINE (9b.4–9b.15 — ดู 0h), ERP item master (9b.22 — ดู 0h.15) — คำถาม blocking เรื่อง LLM provider/data policy หมดไปพร้อมการตัดขอบเขต chat ตั้งแต่รอบ 11 (ดู 0e/0g) — เหลือเฉพาะ **9b.23–9b.25 (รายละเอียด Telegram)** ซึ่งไม่ blocking การเริ่มงานส่วนอื่น
...
6. **Responsive + LINE/Telegram (ดู 0d/0g) แนะนำทำหลังสุด** — ทำ desktop flow หลักให้ใช้งานได้ครบก่อน ค่อยทำ mobile layout เฉพาะจุด (โดยเฉพาะ PM/compare ที่ต้องออกแบบใหม่ตาม pattern การ์ด/accordion — ✅ ปิดแล้ว 0h.9) แล้วค่อยต่อ LINE/Telegram เข้ากับ event notification ที่มีอยู่แล้ว (ทำ adapter กลางก่อน แล้วเสียบ LINE ก่อน — OA ใหม่ + LINE Login + push-only ตาม event ที่ต้อง action ✅ ปิดครบแล้ว 0h.10–0h.14 — แล้วค่อยเสียบ Telegram โดยไม่ต้องรื้อ adapter) — ปิดคำถาม 9b.23–9b.25 (Telegram) ก่อนเริ่มจริงจัง
7. **Data migration (ดู 0h.7)** — วางแผนคู่ขนานกับข้อ 3–6 ให้เสร็จก่อนวันตัด switch จริง (โครงการที่ยังเปิดอยู่ในระบบเดิมต้อง migrate ให้ครบ)
...
**`project.entry_file`** — `id, revision_id FK, seq_no, file_path TEXT (✅ path ของ local disk บน server on-prem — ปิดแล้ว ดู 0h.2), file_name TEXT (ชื่อเดิมที่ user ตั้ง), file_size_kb INT, file_type TEXT, is_active, audit`
...
8. ช่องต้นทุน @ ใน template มีการพิมพ์เป็นนิพจน์คำนวณ (`=294.893*3` = จำนวนแบต × ราคาต่อลูก) — ✅ **ปิดแล้ว 7 ส.ค. 2026 (ดู 0h.16): ไม่รองรับพิมพ์นิพจน์ในช่องเงิน** — แยกเป็นช่อง `qty × unit price` ย่อยให้กรอกแทน (ปลอดภัยกว่า ไม่ต้อง parse expression ที่ผู้ใช้พิมพ์เอง และ validate ได้ง่ายกว่า)
...
### A.11b `project.interface_api_logs` — log การเรียก external interface แบบ generic (**ใหม่ 7 ส.ค. 2026 — ✅ 0h.14**)
...
แทนโฟลเดอร์ `Deploy_SQL/<วันที่>/` เดิม — ✅ **ใช้ Prisma Migrate (ปิดแล้ว ดู 0h.1)**, เนื้อหาที่ต้อง seed เหมือนเดิมทั้งหมด: 13 สถานะ, 10 ทีม, ยี่ห้อคู่แข่ง, ประเภทหน่วยงาน, เหตุผลแพ้/สาเหตุล่ม, `notification_config` เริ่มต้น (`near_due_days = 90`), index ต่างๆ (`registration(org_name_norm)`, `registration(project_name_norm)`, `entry(project_id)`, `entry(sale_user_name, status_id)`, unique partial `entry_revision(entry_id) WHERE is_current_revision`, UNIQUE `entry_revision(entry_id, revision_no)`, `notification(target_user_name, is_read)`, `dealer(name)` (+ `pg_trgm` GIN index), `team_user(user_name)`, `interface_api_logs(interface_name, response_status, created_at)`)

### A.15 ~~AI Chat Assistant — conversation storage~~ → ✂️ ตัดออก (30 ก.ค. 2026 — ดู 0e/0g)

ไม่มีตารางเพิ่มจากหัวข้อนี้ — ช่องทาง push ใช้คอลัมน์บน `auth.user` (A.13) เท่านั้น ไม่มีตารางใหม่ — **ถ้ารื้อกลับมาทำใน Phase ถัดไป:** แนวคิดตั้งต้นคือ `project.chat_session` (1 แถวต่อการเปิดแชท, key คือ `user_name` จาก JWT) และ `project.chat_message` (1 แถวต่อข้อความ พร้อม `tool_calls JSONB` สำหรับ debug/audit — ไม่ทำ FK ตรงไปยัง `registration`/`entry` เพราะข้อความเป็น free text) พร้อม retention ที่กำหนดผ่านตาราง config แบบ `lov_config` แทนการ hardcode — `pgvector` extension มีพร้อมใช้งานระดับ infra แล้วเผื่อวันนั้นต้องการ RAG (รายละเอียดเต็มอยู่ใน git history commit `748d2bc`)
...
- Scenario ทดสอบ: D13 (role-level), D18 (matrix ทีม), D28 (record-level + ปลอมตัวตนใน body) — ไม่มี endpoint chat ในเฟสนี้ จึงไม่มี scenario role-leakage ผ่านแชทให้ทดสอบ (ดู 0e/0g)
...
| D30 | **(ดู 0d.1/0d.2 — ✅ ทิศทาง design ยืนยันแล้ว 0h.9, ยังต้องมี mockup ละเอียดก่อน build)** เปิดหน้า ProjectRegister ALL / To-do list บนจอมือถือ | ข้อมูล/ฟังก์ชันครบเหมือน desktop ทุกอย่าง (sort/filter/paging เรียก endpoint เดียวกัน) แต่แสดงผลเป็น card list แทนตารางแนวนอน — ไม่มีคอลัมน์ไหนหายไปจากที่ desktop มี เทียบกันแล้วแค่จัด layout ต่างกัน |
| D31 | **(ดู 0d.3/0g — ✅ กลไกฝั่ง LINE ยืนยันแล้ว: OA ใหม่ + LINE Login OAuth + push-only ดู 0h.10–0h.13; Telegram ยังเปิดอยู่ 9b.23–9b.25)** ผู้อนุมัติ 3 คน: คนที่ผูก LINE, คนที่ผูก Telegram, คนที่ยังไม่ผูกช่องทางใดเลย — มีคำขอ "รออนุมัติ" ใหม่เข้ามาในทีม | คนที่ผูก LINE ได้ Flex Message / คนที่ผูก Telegram ได้ข้อความพร้อม inline keyboard — **เนื้อหาเท่ากันทั้ง 2 ช่องทาง (project code + ประเภทคำขอ ไม่มีราคา/GP ตาม 8.18)** และปุ่ม deep-link เปิดไปหน้า Approval Detail ของรายการนั้นตรงๆ; คนที่ยังไม่ผูก **ไม่ได้รับ push เลยแต่กระดิ่งในแอปยังเห็นปกติ**; push ล้มเหลวช่องทางใดช่องทางหนึ่ง (เกิน quota/บล็อก bot/ผู้ใช้ลบแชท) **ไม่ทำให้การสร้างคำขออนุมัติล้มเหลว และไม่ทำให้ช่องทางอื่นไม่ถูกส่ง** — บันทึกผลการส่งทุกครั้งลง `project.interface_api_logs` (0h.14) |
