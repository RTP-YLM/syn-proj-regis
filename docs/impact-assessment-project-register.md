...
[React SPA]  ──HTTPS/JSON──▶  [Fastify API]  ──SQL (driver/ORM)──▶  [PostgreSQL]
 ...
 เรียก API ผ่าน fetch/axios    ต่อ endpoint                          สำหรับ auth.user)
     ...
     │                              │        │
     │  [Chat widget] ──HTTPS/JSON──┴──▶ [Chat orchestration] ──prompt+tool result──▶ [LLM API ภายนอก] (0e)
     │   ในหน้า React                    (Fastify, ดู 5)              provider ยังไม่เลือก — ดู 9b.16
...
- **สิ่งที่ต้องตัดสินใจก่อนเริ่ม coding จริง** (ไม่ใช่แค่ "แนะนำ" แต่ blocking): ORM/query layer, ที่เก็บไฟล์แนบ (disk vs object storage), responsive breakpoint/pattern สำหรับตารางข้อมูลหนาแน่น, LINE OA/การผูกบัญชี, **LLM provider + นโยบายข้อมูลอ่อนไหว (9b.16)** — รวมไว้ในหัวข้อ 9b เป็นคำถามคงค้าง (SSO ปิดแล้ว ดู 9c)
...
### 4.5 AI Chat Assistant — Chat widget (ใหม่ 22 ก.ค. 2026 — ดู 0e)
...
- **ยังไม่ตัดสินใจ**: ต้อง stream คำตอบทีละ token (เหมือน ChatGPT พิมพ์ทีละตัว) หรือรอครบแล้วแสดงทีเดียวพอ — กระทบการเลือก transport ฝั่ง Fastify (ดู 9b.18)
...
| **(ใหม่ 22 ก.ค. 2026 — ดู 0e) Chat orchestration** | endpoint ใหม่ `POST /chat/message` (ดู B ข้อ 26) — รับข้อความ user + `sessionId` แล้ว: (1) เรียก LLM API พร้อม **tool/function definitions** ที่ map ตรงกับ endpoint อ่านข้อมูลที่มีอยู่แล้ว (`/list`, `/detail`, `/compare`, `/notification`, `/approve-list` ฯลฯ) — ให้ LLM เลือกเรียก tool ที่เหมาะสมแทนการ query DB เองตรงๆ; (2) **ทุก tool call ต้องผ่าน authorization layer เดียวกับ REST endpoint ปกติทุกจุด** (Access Matrix ข้อ B.2) — LLM ไม่มีสิทธิ์พิเศษเหนือกว่า role ของ user ที่ login อยู่ (กัน chat กลายเป็นช่องทางหลุด record-level authorization ที่ตั้งใจคุมไว้แล้วในหัวข้ออื่น — ดูความเสี่ยง 8.21); (3) เอาผลลัพธ์ JSON จริงจาก DB ส่งกลับให้ LLM format เป็นคำตอบภาษาธรรมชาติ (ไม่ให้ LLM สร้างตัวเลข/ข้อเท็จจริงเอง — ดู 8.20); (4) เก็บ conversation history (ดู 6.5) เพื่อรองรับ multi-turn (FR-10) |
| **(ใหม่) Aggregate/stats endpoint** | endpoint ใหม่รองรับ FR-07 (win rate, GP เฉลี่ย, มูลค่ารวมต่อช่วงเวลา) — ของเดิมไม่มี endpoint ระดับ aggregate ขนาดนี้มาก่อน (หน้าจอเดิมแสดงเป็นรายการ ไม่ใช่สรุปตัวเลข) — คำนวณสดจาก query (`GROUP BY`/`SUM`/`AVG`) ก่อน ยังไม่ต้องทำ materialized view จนกว่าจะรู้ปริมาณข้อมูลจริง (ดู 9b.20) |
...
### 6.5 AI Chat Assistant — conversation storage (ใหม่ 22 ก.ค. 2026 — ดู 0e)

ตารางใหม่ **schema `project`** (ไม่ใช่ schema แยก — เป็นข้อมูลของฟีเจอร์นี้เหมือนกัน) รองรับ conversation context (FR-10) และ audit trail ของคำถาม/คำตอบที่อาจมีข้อมูลอ่อนไหวปนอยู่:

- `project.chat_session` — 1 แถวต่อการเปิดแชท 1 รอบ: `id, user_name TEXT (= identity จาก JWT), started_at, last_message_at` + audit
- `project.chat_message` — 1 แถวต่อข้อความ: `id, session_id FK, role TEXT ('user'/'assistant'), content TEXT, tool_calls JSONB NULL (เก็บว่า LLM เรียก tool ไหนด้วย parameter อะไร — ใช้ debug/audit), created_at`

**ยังไม่ตัดสินใจ**: เก็บถาวรกี่วัน/ลบอัตโนมัติเมื่อไร (เนื้อหาข้อความอาจมีตัวเลขต้นทุน/GP ที่หลุดจากหน้าจอปกติไปอยู่ในข้อความแชท — เป็น attack surface ใหม่ที่ต้องคุม retention/สิทธิ์การเข้าถึง log เหมือนข้อมูลอ่อนไหวอื่นในระบบ ไม่ใช่ log ทั่วไป) — ดู 9b.17 และความเสี่ยง 8.23
...
| 8.19 | **(ใหม่ 22 ก.ค. 2026 — ดู 0e)** ข้อมูลต้นทุน/GP/ราคา/ชื่อลูกค้าที่ Sales ทุกคนเห็นอยู่แล้ว (8.1) ถูกส่งออกไปยัง **LLM API ภายนอกองค์กร** ทุกครั้งที่มีคำถามเกี่ยวข้อง ถ้าใช้ third-party cloud LLM ทั่วไปโดยไม่มีข้อตกลงประมวลผลข้อมูล (DPA)/ไม่ opt-out การใช้ข้อมูลไปเทรนโมเดล | **สูง** | ต้องยืนยันนโยบายกับผู้ใช้/ฝ่าย security ก่อน implement จริง (ไม่ใช่แค่แนะนำ) — ตัวเลือก: (1) enterprise API tier ที่มี DPA + no-training guarantee, (2) self-hosted/private endpoint (เช่น โมเดล open-weight รันในองค์กรเอง หรือ VPC-hosted managed LLM), (3) ลด field ที่ส่งเข้า prompt ให้น้อยที่สุด (ตัด cost/GP ออกจาก context ถ้าคำถามไม่จำเป็นต้องใช้) — ดูคำถาม 9b.16 (blocking) |
| 8.20 | **(ใหม่)** LLM อาจ "hallucinate" — ตอบตัวเลข/ข้อเท็จจริงที่ฟังดูมั่นใจแต่ผิดจากข้อมูลจริงใน DB (อันตรายเป็นพิเศษกับตัวเลข GP/ราคาที่ใช้ตัดสินใจธุรกิจ) | สูง | บังคับ architecture แบบ tool-use เท่านั้น (ดู 0e.2/5) — LLM ห้ามสร้างตัวเลขเอง ต้องอ้างจากผลลัพธ์ tool call ที่ query จาก DB จริงเสมอ, พิจารณาแสดงแหล่งอ้างอิง/ปุ่ม deep-link ควบคู่ทุกคำตอบที่มีตัวเลขเพื่อให้ user เช็คย้อนกลับได้ |
| 8.21 | **(ใหม่)** Chat เป็นช่องทางใหม่ที่อาจ**หลุด record-level authorization** ที่ระบบอื่นคุมไว้แล้ว (Access Matrix ข้อ B.2) — เช่น headsale ถามอ้อมๆ จนได้ข้อมูลทีมอื่นที่ปกติเข้าเมนูตรงๆ ไม่เห็น หรือ prompt injection หลอกให้ LLM เพิกเฉยต่อขอบเขตสิทธิ์ | สูง | **ทุก tool call ต้องบังคับ authorization ที่ชั้น server เดียวกับ REST endpoint เป๊ะๆ ไม่ผ่าน LLM เป็นคนตัดสิน** (LLM แค่เลือกว่าจะเรียก tool ไหน ไม่ใช่คนกำหนดว่าเรียกได้หรือไม่ได้) — ต้องมี test เฉพาะสำหรับ role-leakage ผ่านแชท (ดู scenario ใหม่ในภาคผนวก D) |
| 8.22 | **(ใหม่)** ต้นทุนใช้งาน LLM API เป็นรายครั้ง/token — ถ้าไม่มี rate limit อาจโดนใช้เกินคาด (ตั้งใจหรือไม่ตั้งใจ) + latency ต่อคำตอบช้ากว่าโหลดหน้าปกติมาก ผู้ใช้อาจคาดหวังเร็วเท่าเมนูอื่น | กลาง | จำกัด rate ต่อ user/ทีม (ดู 9b.19), แสดง loading indicator ชัดเจน (ดู 4.5), พิจารณา cache คำตอบสำหรับคำถามซ้ำๆ ที่ข้อมูลยังไม่เปลี่ยน |
| 8.23 | **(ใหม่)** Log การสนทนา (`chat_message`) อาจมีตัวเลขต้นทุน/GP/ชื่อโครงการปนอยู่ในข้อความเต็ม — กลายเป็นแหล่งข้อมูลอ่อนไหวอีกจุดที่ต้องคุมสิทธิ์การเข้าถึงแยกจากที่คุมไว้แล้วในหน้าจอปกติ | กลาง | ต้องมีนโยบาย retention + จำกัดคนดู log ได้ (เช่น เฉพาะ admin หรือดูได้แค่ของตัวเอง) — ไม่ใช่ log debug ทั่วไปที่ dev เข้าดูได้อิสระ (ดู 6.5/9b.17) |
| 8.24 | **(ใหม่)** ภาษาไทยมีคำย่อ/การสะกดหลากหลาย (เช่น "รพ." "ร.พ." "โรงพยาบาล") และพิมพ์ผิดได้ง่าย — LLM อาจตีความหมายผิดโดยไม่รู้ตัว (ต่างจาก error ที่เห็นชัดแบบ query ปกติ) | กลาง | ทดสอบชุดคำถามภาษาไทยจริงจากผู้ใช้ Sales/หัวหน้าก่อนปล่อยใช้งานจริง ไม่ใช่แค่ทดสอบด้วยตัวอย่างในเอกสาร FR เท่านั้น — ถ้า fallback (0e.11) ทำงานถูกต้อง ความเสี่ยงนี้ลดจาก "ตอบผิดแบบไม่รู้ตัว" เหลือแค่ "หาไม่เจอแล้วแนะนำลองใหม่" ซึ่งปลอดภัยกว่ามาก |
...
| 9b.16 | **(ใหม่ 22 ก.ค. 2026 — ดู 0e)** LLM provider ตัวไหน (OpenAI/Anthropic/Azure OpenAI/self-hosted open-weight ฯลฯ) + มีข้อตกลงประมวลผลข้อมูล (DPA)/no-training guarantee หรือไม่ | ยังไม่ระบุ — **blocking ที่สุดในบรรดาคำถามรอบนี้** เพราะกระทบว่าข้อมูลต้นทุน/GP ส่งออกนอกองค์กรได้แค่ไหน | บล็อกการเริ่ม implement ชั้น chat orchestration ทั้งหมด (ดูความเสี่ยง 8.19 — สูงสุดในรอบนี้) — ต้องปิดก่อนคำถามอื่นในกลุ่ม 9b.17–9b.21 |
| 9b.17 | **(ใหม่)** เก็บ conversation history ไว้ถาวรใน DB (audit ได้ แต่เพิ่มพื้นที่เก็บข้อมูลอ่อนไหว) หรือเก็บแค่ session ปัจจุบัน (ephemeral, หายเมื่อปิดแท็บ/refresh) | สมมติ **เก็บใน DB แบบมี retention policy** (ดู 6.5) เพื่อรองรับ multi-turn ข้ามการ refresh หน้า — ยังไม่กำหนดจำนวนวัน | กระทบ schema `chat_session`/`chat_message` (6.5) และนโยบายเข้าถึง log (ความเสี่ยง 8.23) |
| 9b.18 | **(ใหม่)** ต้อง stream คำตอบทีละ token หรือรอครบข้อความแล้วส่งทีเดียว | สมมติ **request/response ธรรมดาก่อน** (ไม่ทำ SSE/WebSocket รอบแรก) — ง่ายกว่า implement แต่ผู้ใช้ต้องรอเห็นคำตอบพร้อมกันทั้งก้อน (latency รู้สึกนานกว่า) | กระทบการเลือก transport ฝั่ง Fastify (ดู 4.5) — เปลี่ยนเป็น streaming ทีหลังได้โดยไม่กระทบ schema/tool-call logic |
...
| **(ใหม่ 22 ก.ค. 2026 — ดู 0e/5) Chat orchestration layer** (LLM API integration, tool/function definitions ผูกกับ endpoint อ่านข้อมูลที่มีอยู่แล้วทั้งหมด, บังคับ authorization ต่อ tool call, prompt design กัน hallucination) | 8–12 |
| **(ใหม่ — ดู 0e/6.5) Conversation storage + aggregate/stats endpoint ใหม่** (`chat_session`/`chat_message` + endpoint FR-07) | 3–4 |
| **(ใหม่ — ดู 0e/4.5) Chat widget UI** (panel, message bubble, การ์ดผลลัพธ์มีโครงสร้าง, ปุ่ม deep-link, responsive) | 4–6 |
| **(ใหม่ — ดู 0e) Security/policy work เฉพาะ chat** (ยืนยัน DPA/data policy กับ LLM provider, ออกแบบ retention ของ conversation log, ทดสอบ role-leakage ผ่านแชท) | 3–5 |
| ทดสอบรวม + แก้บั๊ก + UAT (เพิ่ม flow Manager + แพ้/ล่ม + auth ผ่าน SSO — D29 + responsive/LINE — D30–D31 + AI chat — D32–D35) | 9–13 |
| **รวม (รวม Auth/SSO + Mobile/LINE + AI Chat Assistant แล้ว — ไม่รวม ramp-up stack ใหม่)** | **~95–143 man-day** |

> ตัวเลขเป็น effort รวม UI+API ถ้าทำขนาน 2 คน (UI/API) ระยะเวลาปฏิทินประมาณ **14–18 สัปดาห์รวม UAT** (รวม Auth/SSO + Mobile/LINE + AI Chat แล้ว — ยังไม่รวม ramp-up stack ใหม่ถ้ามี) — ประวัติการปรับตัวเลขรอบก่อนหน้า: รอบ 2 เพราะ updated-flow ดึงขั้น Manager/พี่บีเข้ามาทั้งหมด (~43–65 → ~54–78 man-day); รอบ 3/4 (review) เพิ่ม Project-level lifecycle, revision อนุมัติซ้ำ, event notification (~54–78 → ~59–86 man-day); รอบ 5 (re-platform) เพิ่มงาน design system ใหม่ แต่ตัด/ปรับงานที่เคย workaround เฉพาะ .NET ออก (เช่น file upload ผ่าน MVC action) — สุทธิใกล้เคียงเดิม (~59–86 → ~61–90 man-day) บวก Auth/SSO ที่ตอนนั้นยัง estimate ไม่ได้; รอบ 6 (ปิด SSO): เพิ่ม Auth/SSO integration 6–9 man-day เข้ารวมยอด (~61–90 → ~67–99 man-day); รอบ 7 (Mobile + LINE): เพิ่ม Responsive 5–8 + LINE integration 4–6 man-day เข้ารวมยอด (~67–99 → ~76–113 man-day) — ตัวเลข Mobile/LINE นี้ยังหยาบกว่าส่วนอื่นเพราะคำถาม 9b.11–9b.15 ยังไม่ปิดเลยสักข้อ; **รอบ 8 (AI Chat Assistant):** เพิ่ม Chat orchestration 8–12 + Conversation storage/aggregate 3–4 + Chat UI 4–6 + Security/policy 3–5 man-day เข้ารวมยอด (~76–113 → ~95–143 man-day) — ก้อนงานใหญ่ที่สุดในบรรดา 3 รอบขยายขอบเขตหลังปิด business spec (0d/0e) เพราะเป็น subsystem ใหม่ทั้งชุด (LLM orchestration + conversation UI) ไม่ใช่แค่ integration จุดเดียวแบบ LINE — และมีคำถาม blocking (9b.16 — LLM data policy) ที่ต้องปิดก่อนเริ่ม ไม่ต่างจาก SSO เดิมที่เคย block ทั้งชุด — ยังไม่รวม ramp-up stack ใหม่ถ้าทีมไม่คุ้นเคย React/Node/Postgres/LLM integration (ความเสี่ยง 8.12 ยังไม่ทราบคำตอบ)

### ลำดับงานแนะนำ

0. **ปิดคำถาม 9b ที่ยัง blocking** — ตอนนี้เหลือหลักๆ คือ ORM (9b.4), design system (9b.7), และ **LLM provider/data policy (9b.16 — blocking chat orchestration ทั้งชุด)** — **SSO (9b.1–9b.3) ปิดแล้ว (ดู 9c) ไม่ block อีกต่อไป**
...
6. **Responsive + LINE (ดู 0d) แนะนำทำหลังสุด** — ทำ desktop flow หลักให้ใช้งานได้ครบก่อน ค่อยทำ mobile layout เฉพาะจุด (โดยเฉพาะ PM/compare ที่ต้องออกแบบใหม่) แล้วค่อยต่อ LINE เข้ากับ event notification ที่มีอยู่แล้ว — ปิดคำถาม 9b.11–9b.15 ก่อนเริ่มจริงจัง (ยังไม่ปิดข้อไหนเลย ต่างจาก SSO ที่ปิดแล้วก่อนเริ่ม 5)
7. **(ใหม่) AI Chat Assistant (ดู 0e) ทำหลังสุดของทั้งหมด** — ต้องมี REST endpoint อ่านข้อมูลของ flow หลัก (ข้อ 3–4) ให้ครบก่อน เพราะ chat ทั้งชุดออกแบบให้ "ห่อ" endpoint เดิมด้วย tool-use ไม่ใช่สร้างช่องทางอ่านข้อมูลใหม่ — ปิดคำถาม 9b.16 (LLM provider/data policy) ก่อนเริ่มเขียนโค้ดจริง เพราะกระทบว่าข้อมูลอ่อนไหวส่งออกไปไหนได้บ้าง (blocking เหมือนที่ SSO เคย block auth ทั้งชุด)
...
### A.15 AI Chat Assistant — conversation storage (ใหม่ 22 ก.ค. 2026 — ดู 0e/6.5)
...
ไม่มี FK ผูกกับ `project.registration`/`project.entry` โดยตรง (ข้อความเป็น free text อ้างถึงหลายโครงการในข้อความเดียวได้) — ถ้าต้องการ audit ว่า session ไหนเคยเห็นข้อมูลโครงการใดบ้าง ให้ derive จาก `tool_calls` JSONB แทนการทำ FK ตรงๆ
...
| 26 | POST | `/chat/message` **[ใหม่ 22 ก.ค. 2026 — ดู 0e]** | Chat widget | รับ `{ sessionId? , message }` — สร้าง `chat_session` ใหม่ถ้าไม่ส่ง `sessionId` มา, บันทึก `chat_message` role `user`, เรียก LLM พร้อม tool definitions ที่ map กับ endpoint อ่านข้อมูลข้อ 1–4/7/9/14/14b (ทุก tool call ผ่าน authorization เดียวกับ B.2), บันทึกคำตอบเป็น `chat_message` role `assistant` แล้วส่งกลับพร้อม deep-link reference (`{ projectId, entryId }` ต่อโครงการที่พูดถึง — ให้ React สร้างปุ่มลิงก์เอง ไม่ hardcode URL จาก LLM) |
| 26b | POST | `/chat/session/history` **[ใหม่]** | Chat widget (เปิดแชทซ้ำ) | คืนรายการข้อความของ `sessionId` ที่ระบุ — **เฉพาะเจ้าของ session** (`chat_session.user_name` = identity ที่ verify แล้ว) เท่านั้น |

ฝั่ง React เก็บ base URL ของ API ผ่าน environment variable (เช่น `.env` → `VITE_API_BASE_URL`) แทน `appsettings.json > ApiUrl:ProjectRegister` เดิม / การยิง webhook แจ้งเตือน **pending** (ข้อ 9.15) — เมื่อ requirement ชัดค่อยทำเป็น scheduled job/worker ฝั่ง API (ดูหัวข้อ 5) / **LINE Flex Message push (ข้อ 0d.3) เป็น outbound service ภายใน ไม่ใช่ endpoint ที่ client เรียก** — trigger จากการเขียนแถวใหม่ใน `project.notification` (ดู 5/6.4) / **`/chat/message` (ข้อ 0e) เรียก LLM API ภายนอกเป็นส่วนหนึ่งของการประมวลผล request — latency สูงกว่า endpoint อื่นทั้งหมดในเอกสารนี้ตามธรรมชาติ (ดูความเสี่ยง 8.22)**
...
| `/config/**` | ❌ | ❌ | ❌ | ✅ | — |
| `/chat/message`, `/chat/session/history` **[ใหม่ — ดู 0e]** | ✅ | ✅ | ✅ | ✅ | ตัว endpoint เปิดให้ทุก role เรียกได้ (ทุกคนถามได้) แต่**ข้อมูลที่ตอบกลับต้องถูก scope โดย tool call ภายในตามแถวอื่นในตารางนี้เป๊ะๆ** — เช่น sales ถามเรื่องทีมอื่น ต้องได้ผลเหมือนเรียก `/list` ตรงๆ ด้วย role เดียวกัน (ว่าง/ถูกปฏิเสธ) ไม่ใช่ได้คำตอบเพราะ "ถามผ่านแชท"; `/chat/session/history` จำกัดแค่ session ของตัวเอง |
...
- **หลักการเฉพาะ chat (ดู 0e/8.21):** LLM มีหน้าที่แค่ "เลือกเรียก tool ไหน" ไม่ใช่ "ตัดสินว่าเรียกได้ไหม" — การตัดสินสิทธิ์เกิดที่ชั้น tool-call handler ซึ่งก็คือ service layer เดียวกับที่ REST endpoint ปกติเรียกอยู่แล้ว (ไม่เขียน authorization logic ซ้ำชุดใหม่เฉพาะ chat)
...
| D32 | **(ใหม่ 22 ก.ค. 2026 — ดู 0e, สมมติฐานยังไม่ยืนยัน)** headsale ของทีม A ถามแชทเกี่ยวกับโครงการของทีม B (เช่น "ทีม B มีงานอะไรบ้าง") | ได้คำตอบเหมือนเรียก `/list` ตรงๆ ด้วย role/identity เดียวกัน — **ไม่ได้เห็นข้อมูลทีม B เพิ่มขึ้นเพราะถามผ่านแชท** (ไม่ว่าจะถามตรงๆ หรือพยายามอ้อมด้วยการถามเป็นภาษาธรรมชาติหลายแบบ) — ทดสอบทั้ง sales/headsale/salemanager เทียบกับสิทธิ์จริงตาม B.2 |
